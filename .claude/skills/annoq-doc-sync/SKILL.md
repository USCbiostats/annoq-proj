...
| **Local database containers belong to annoq-database only** (api-v2 has no compose file; both once used container name `elasticsearch` with different volumes) | annoq-proj (architecture, pipeline, repositories, glossary, config skill, `.github/ISSUE_TEMPLATE/config_update.md`); annoq-api-v2 README/CLAUDE.md; annoq-database README |
