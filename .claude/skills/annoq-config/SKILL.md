...
> **Stage 4 is split by stack.** `annoq-site-v2` (React) serves **annoq.org (HRC)**; `annoq-site`
> (Angular 9) serves **topmed.annoq.org (TOPMed)**. A stage-4 config value targets **one** of them
> — pick by URL, not by habit.
...
- **api-v2 endpoint URL** — **both stage-4 site repos** (annoq-site-v2, annoq-site) **and every
  consumer** (annoq-py, AnnoQR, SNPWay) must point at the intended API (dev vs prod). AnnoQR
  defaults to `enrichment-dev.annoq.org` — an easy way to accidentally hit the dev API.
...
| Consumer endpoint (annoq-py / AnnoQR / SNPWay) | Run a sample query/workflow; confirm it hits the intended api-v2 (watch AnnoQR's dev default) |
| Annotation tree | Confirm api-v2 exposes the new tree and **both** sites render it (annoq.org, topmed.annoq.org) |
...
- Endpoint URL change → verify the site build for that environment, in the **site repo that
  serves that stack** (annoq-site-v2 for HRC, annoq-site for TOPMed).
...
- Version-sensitive configs: Elasticsearch 8.5.x, Python 3.11+, Angular 9 (annoq-site),
  Node 20+ (annoq-site-v2) — verify compatibility.
- Editing the config of the wrong **site repo** — annoq.org is annoq-site-v2, topmed.annoq.org is
  annoq-site.
